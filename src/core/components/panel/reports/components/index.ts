export * from './turist-card'
export * from './switch-month-year'
...
export const genRandomColors  = (numColors: number) => {
  const {floor,random} = Math
  const ans:number[] = [];
  let ix:number
  do{
    ix = floor(random() * len);
    if(!ans.includes(ix)) ans.push(ix);
  }while(ans.length < numColors)
    
  return ans.map( i => colors[i] );
}
  
export const genSingleColor = () => {
  const {floor,random} = Math
